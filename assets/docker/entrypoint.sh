...
if [ ! -f ${DATA}/${DS}/RDF/${DS}.hdt ]; then
    ...
    java -jar wp4-links-0.0.1-SNAPSHOT-jar-with-dependencies.jar --inputData ${DATA}/${DS}/RDF/${DS}.nq --outputDir ${DATA}/${DS}/RDF --function convertToHDT  2>&1 | tee -a ${LOG}
    ERR="${?}"
    if [ ${ERR} -ne 0 ]; then
        exit ${ERR}
    fi
...
java -jar wp4-links-0.0.1-SNAPSHOT-jar-with-dependencies.jar --inputData ${DATA}/${DS}/RDF/${DS}.hdt --outputDir ${DATA}/${DS} $*  2>&1 | tee -a ${LOG}
ERR="${?}"
echo " END [`date`] ${*}" >> ${LOG}
